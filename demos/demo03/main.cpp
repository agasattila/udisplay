...
/*
 * demo03 — uDisplay hardware-less TCP device emulator (C++ modern API).
 *
 * Same simulation as demo01, rewritten using the generated C++ header
 * with std::function lambda event handlers (--lang cpp --modern).
 * TCP transport is handled by demos/shared/demo_tcp.
 *
 * Usage:  ./demo03 [port]   (default port: 5555)
 */
...
int main(int argc, char* argv[])
{
    ...
    if (argc >= 2) {
        ...
        if (port <= 0 || port > 65535) {
            fprintf(stderr, "usage: demo03 [port]\n");
            return 1;
        }
    }
    ...
    ui.text_input.on_submit = [](const char* str, uint8_t len) {
        char buf[256];
    ...
    printf("[demo03] listening on port %d\n", port);
    printf("[demo03] connect with:  udisplay-client tcp://127.0.0.1:%d\n", port);
    printf("[demo03] press Ctrl+C to exit\n");
